...
- **CLAUDE.md 模板路径**：E:\claudeCode\DOC（例：E:\skills\CLAUDE.md）

---

## Skill 路径

- **Skill Index 文件夹**：E:\claudeCode\DOC\skills\migration（例：E:\skills\port，自动找 00-INDEX.md）
- **引擎规范文件夹**：E:\claudeCode\DOC\DEV_RULES（例：E:\skills，根据目标引擎自动匹配对应规范文件）
- **视觉规范文件夹**：E:\claudeCode\DOC\VISUAL_RULES（例：E:\skills，根据目标引擎自动匹配，没有就写"无"）
...
（例：C:\Unity\Unity.exe，根据你安装的引擎填写）
...
贴图在美术资产路径的的cards 和 coins 文件夹下面，注意查看
...
### 步骤 4 — 写入 Memory
**必须写入以下 6 条规则到 memory，即使 CLAUDE.md 已有类似内容也必须写入，这是两个不同的机制：**
> 1. 每个 Phase 完成后必须执行 8 步收尾，详见 CLAUDE.md
> 2. 测试不可跳过，全绿才能继续
> 3. 不得依赖记忆，必须读取文件
> 4. TDD：垂直切片，测试行为不测实现，不 Mock 自己控制的代码
> 5. 引擎问题查引擎规范文件，不靠记忆
> 6. 每个 Phase 完成后必须确保项目没有编译错误或崩溃，具备本 Phase 完整功能。然后停下来告知用户：'✅ Phase X 完成，请在 [引擎名] 里打开项目运行测试，确认没问题后告诉我继续。'
