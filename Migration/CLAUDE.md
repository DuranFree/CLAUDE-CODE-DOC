...
## 规则标记机制（DEV_RULES / VISUAL_RULES 里会用到）

读取三引擎规则文件（`DEV_RULES/*.md`、`VISUAL_RULES/*.md`）时，遇到以下两种标记必须按对应动作处理：

- **`⚠️ [写入CLAUDE.md]`** — 该规则是硬代码规则（违反立即出 bug / 编译错误 / 性能问题），必须复制到项目根目录 `CLAUDE.md` 的"引擎常见坑"段落，让每次会话都强约束
- **`⚠️ [写入memory]`** — 该规则是工作流程 / 行为模式 / 偏好提醒，必须在项目 memory 目录新增一条 `feedback_*.md` 并在 `MEMORY.md` 索引里登记，让每次会话启动自动加载但不占 CLAUDE.md 权重

两种标记可能单独出现也可能同时出现，按标记逐条处理，**不得省略**。首次读到某条规则时立即执行同步，已同步的跳过。
...
7. **最简实现**：只写被明确要求的功能，不加推测性功能、未请求的抽象或防御性代码。
8. **精准修改**：只改被要求的部分，不顺手"优化"周边代码、注释或格式。（发现真实 bug 除外，见第 1 条）
9. **目标驱动**：每个任务开始前明确可验证的完成标准，达标前不停止。

---

## 任务开工前 — 模型能力匹配检查

每次收到新任务（不是单轮追问），开工前先判断任务类型，若命中以下任一类别，**必须先提示用户切换更高推理强度再开工**，不得直接用默认 medium 执行：

1. **架构 / 技术选型决策** — 多方案权衡、跨系统影响
2. **跨 5+ 文件的重构**
3. **性能优化** — 涉及热路径 / GC / Draw Call 分析
4. **时序 / 生命周期 / 销毁相关 bug** — 跨帧异步回调、对象销毁后时机、竞态条件
5. **视觉效果方案选型** — Shader / VFX / Post-processing 多路线对比
6. **规则 / 工作流 / 元层面设计**
7. **疑难 bug 根因分析**（现有方案治标不治本 / 多次尝试未果）

**提示格式：**
> `⚠️ [模型建议] 此任务属于 <类别>，medium 容易给表面答案。建议切 high / extended thinking 后再继续，还是坚持 medium？`

**静默进入 medium（不触发）：**
- 改明确 bug（有错误信息 / stack trace）
- 按已有方案写代码
- 单文件小改动
- 写测试
- 改 UI 文案 / 颜色 / 布局
...
**不确定归类时：** 列清楚判断依据（任务涉及几个系统？有没有技术权衡？有没有现成方案？）让用户决定，不得默认往低判断。

**原则：** 模型自评能力有限（medium 下的自评本身就有局限），宁可提醒过度也不要漏提醒。
...
需要查询引擎内部状态（场景结构、对象层级、组件属性、资源信息等）时，优先判断是否有对应的 MCP 工具可用：
- 工具可用 → 直接调用 MCP 工具，不读取引擎的原生场景文件
- 工具不可用 → 才读取文件或让用户描述

**注意：** 避免查询顶层大节点，数据量可能极大，应尽量指定具体的子路径或对象名。
