...
- MCP run_tests 出错 / 超时 / 无响应时，**必须按以下顺序排查，不得直接杀编辑器**：
  1. 停止 Play Mode
  2. 确认场景已加载，若为空则用项目指定方式重建场景
  3. 重试 MCP（此时可能触发编译）
  4. 等待编译完成
  5. 如果本次有过 MCP 场景修改 → 调用引擎 MCP 场景保存工具
  6. 再次重试 MCP
...
**MCP 修改场景后必须立即保存：** 任何通过 MCP 修改场景内容的操作（添加/删除/修改对象、组件属性等）完成后，必须立即调用引擎 MCP 的场景保存工具，防止场景处于 dirty 状态，避免后续操作触发引擎保存弹窗导致 MCP 超时。
