...
**第四步 — 引擎常见坑写入项目 CLAUDE.md：**
5. 读取对应引擎的 DEV_RULES 文件（如 `UNITY-RULES.md`）
6. 找出所有标注 `⚠️ [写入CLAUDE.md]` 的条目
7. 将这些条目追加到**当前项目根目录的 `CLAUDE.md`**（不是 DOC 里的通用 CLAUDE.md），放在"引擎常见坑"节下
8. 输出：`✅ [引擎规则] 已将 [N] 条常见坑写入项目 CLAUDE.md`
...
**按项目类型识别，存在则写入监测规则：**
- `assets-index.json` 或类似资源索引文件 → 存在时写入：`如果 assets-index.json 出现在 git status 修改列表中，立即读取它`
- `deep-scan-results.md` — 之后不再修改，无需监测
- 视觉升级指南（`visual-upgrade-*.md`）— 启动后基本不变，无需监测
- 移植计划文件（`port-*.md`）— 启动后基本不变，无需监测
...
**第一步 — 区域存在性检查：**
- 列出原版中所有独立的游戏区域/面板/堆/层（例：手牌区、基地区、战场区、符文区、传奇区、弃牌堆、放逐堆、英雄堆、牌库显示、得分面板、菜单区等——按实际项目调整）
...
- 以原版源码为基准逐一对比（不是对比功能清单——清单本身可能不完整），发现缺失项立即补充到功能清单并告知用户
